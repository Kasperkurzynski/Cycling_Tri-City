...
Projekt został stworzony w celu wykonania wizualizacji na podstawie danych dotyczących ruchu rowerowego w Gdańsku. Ruch rowerowy monitorowany jest za pomocą stacji pomiarowych umiejscowionych w różnych miejscach w mieście. Zakres analizy obejmuje przedział czasowy od 01.10.2013 do 31.03.2021 roku. Projekt zakłada stworzenie wizualizacji na podstawie dziewięciu poleceń. Do pierwszych dwóch poleceń zostaną wykorzystane wykresy z **R Graphics**, natomiast pozostałe polecenia zostaną wykonane przy pomocy pakietu **ggplot2** oraz jego rozszerzeń.

___
## **Utworzenie i modyfikacja zmiennych**

Zostały utworzone zmienne takie jak: *Month*, *Year*, *Day*, *Dzie_tyg*, *Czy_Weekend* oraz *Pora_Roku* na podstawie zmiennej *Data*, a także został zmieniony typ nowych zmiennych na **factor**.
...
Przedstawić rozkład liczby dni pomiarowych w poszeczególnych punktach.
...
ggplot(zad3, aes(x = reorder(Stacja, -Natezenie), y = Natezenie)) + geom_bar(stat="identity", fill = heat.colors(27), alpha = 0.8) +
  ...
  annotate(geom = 'rect',
           ...
           fill = 'pink',
           alpha = 0.6) +
  annotate("text", x = 23.5, y = 500, label = "Stacje o najniższym natężeniu") +
  xlab("Stacja") +
  ylab("Natężenie") +
  ggtitle('Natężenie ruchu rowerowego w zależności od stacji')
```

### Porównanie względem rozkładu dla wybranych stacji
...
Przedstawić natężenie / rozkład przejazdów dla wybranej stacji (**Pas Nadmorski**) w zależności od miesiąca / dnia tygodnia / dni powszednich / weekendowych.
...
ggplot(zad4, aes(x = reorder(Month, -Natezenie), y = Natezenie)) + geom_bar(stat="identity", fill = 'slateblue', alpha = 0.8) +
  ...
  annotate(geom = 'rect',
           ...
           fill = 'green',
           alpha = 0.4) +
  annotate("text", x = 5.5, y = 5400, label = "Miesiące o największym natężeniu") +
  annotate(geom = 'rect',
           xmin = 9.5, xmax = 12.5,
           ymin = 0.5, ymax = 800,
           fill = 'pink',
           alpha = 0.6) +
  annotate("text", x = 10.5, y = 1150, label = "Miesiące o najniższym natężeniu") +
  xlab("Miesiąc") +
  ylab("Natężenie") +
  ggtitle('Natężenie ruchu rowerowego w zależności od miesiąca')
```

### Natężenie a typ dnia i pora roku
...
### Porównanie wszystkich stacji na podstawie średniej liczby przejazdów
...
Dla wybranej stacji (**Pas Nadmorski**) przedstawić zależność między liczbą przejazdów a wybranymi warunkami pogodowymi.
...
Porównać stacje (**wybrane**) pod względem zależności z poprzedniego punktu.

W tym celu została stworzona animacja za pomocą pakietu **gganimate**.
...
### Stacja a średnia liczba przejazdów
...
### Stacja a mediana liczby przejazdów
...
### Stacja a maksymalna liczba przejazdów
...
ggplot(zad9, aes(x = Zachmurzenie, y = Wilgotność)) + geom_hex(bins = 60) + 
  scale_fill_gradient(low = "lightsteelblue1", high = "springgreen3") +
  theme_minimal() + 
  ylab("Zachmurzenie [oktany]") + ylab("Wilgotność [%]") +
...
ggplot(zad9, aes(x = Ciśnienie_stacja, y = Ciśnienie_morze)) + geom_hex(bins = 80) + 
...
ggplot(zad9, aes(x = Temperatura, y = Wilgotność)) + geom_hex(bins = 70) + 
...
ggplot(zad9, aes(x = Temperatura, y = Ciśnienie_woda)) + geom_hex(bins = 70)  + 
...
doKalendarza <- przejazdy[przejazdy$Year == "2019" & przejazdy$Ciśnienie_stacja > 1022.0, ]
